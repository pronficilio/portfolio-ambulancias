...
function achat_actualizaMensajes(){
    $.ajax({
        cache: false,
        url: "panel/achat/ajax/dameMensajes.php",
...
function achat_iniciaChat(){
    $.post("panel/achat/ajax/iniciaChat.php", "nombre="+$("#achat-nombre_chat").val()+"&email="+
           $("#achat-email_chat").val()+"&pregunta="+$("#achat-pregunta_chat").val(), function(data){
        if(data == ""){
...
function achat_iniciaChat2(){
    $.ajax({
        cache: false,
        url: "panel/achat/ajax/iniciaChat.php",
        type: "post",
        data: "nombre="+$("#achat-nombre_chat").val()+"&email="+
              $("#achat-email_chat").val()+"&pregunta="+$("#achat-pregunta_chat").val(),
        success: function(data){
            if(data == ""){
                $("#achat-instrucciones").html("<p class='media-middle'>Gracias</p>");
            }else{
                alert(data);
            }
        } 
...
function achat_preparaChat(){
    var html = '<div id="achat-chat"><p id="achat-cabecera">';
    $.get("panel/achat/ajax/achatCabecera.php", function(cabecera){
        html += cabecera;
        html += '</p><div id="achat-contieneMensajes" class="achat-cuerpo achat-ocultamiento"><div id="achat-instrucciones">';
        $.get("panel/achat/ajax/achatDisponible.json.php", function(disponible){
            if(disponible){
            ...
            html += '</div></div><div id="achat-mandaMensaje"><input type="text" '+
                    'id="achat-mandaMensaje_input" placeholder="Escribe un mensaje..."></div></div>';
            $("body").append(html);
            $.post("panel/achat/ajax/dameMensajes.php", function(data){
                if(data != ""){
                    $("#achat-contieneMensajes").html(data);
                    $("#achat-contieneMensajes").removeClass("achat-ocultamiento");
                    var objDiv = document.getElementById("achat-contieneMensajes");
...
$(function(){
    ...
    $(document).on("keypress", "#achat-mandaMensaje_input", function(e){
        if(e.which == 13) {
            $.post("panel/achat/ajax/mandaMensaje.php", "mensaje="+$("#achat-mandaMensaje_input").val(), function(data){
                console.log(data);
                $("#achat-contieneMensajes").append("<div class='achat-mensaje'>"+
